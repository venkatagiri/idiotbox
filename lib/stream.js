...
stream.prototype.start = function(cb) {
  ...
  ts.on('ready', function() {
    ...
    ts.files.forEach(function(file) {
      ...
      // This file will be streamed
      if(!this.mainFile && file.name.match(STREAMABLE_FORMATS)) {
        this.mainFile = {
          url: ['/stream/', this.id, '/file/', file.id].join(''),
          mimeType: ['video/', file.name.split('.').pop()].join('')  // video/mp4, video/avi
        };
        console.log('StreamId:', this.id, 'MainFile:', file.name);
      }
    }.bind(this));
