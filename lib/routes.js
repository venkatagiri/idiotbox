  ...
  app.get('/video/:streamId', function(request, response) {
    ...
    factory.getStream(streamId, function(err, stream) {
      if(err) {
        ...
        stream.getVideo(function(err, video) {
          if(err) {
            request.session.msg = err;
            response.redirect('/');
          } else {
            response.set('Content-Type', video.mimeType);
            response.set('Content-Length', video.length);
            video.file.createReadStream().pipe(response);
          }
        });
      }
    });
